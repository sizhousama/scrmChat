...
/**
 ...
 export function parseMsg(fbMsg) {
   ...
   if (entry[0].messaging) {
     const { messaging, id } = entry[0]
     const { message, recipient, sender, timestamp } = messaging[0]
     ...
     if (!hasProperty(messaging[0], 'postback')) {
       ...
       const { title } = messaging[0].postback
       msgItem.type = 'postback'
       msgItem.text = title
       return msgItem
     }
   } else {
     const { standby, id } = entry[0]
     const { message, recipient, sender, timestamp } = standby[0]
     ...
     if (!hasProperty(standby[0], 'postback')) {
       ...
       const { title } = standby[0].postback
       msgItem.type = 'postback'
       msgItem.text = title
       return msgItem
     }
 ...
 export function parseImgMsg(originMsg) {
   const { attachments, text = '' } = originMsg
 ...
 export function parseBtnMsg(originMsg) {
   ...
   const { attachments, text } = originMsg
   const { payload = {}} = attachments[0]
   const { template_type = '', buttons = [] } = payload
   msgItem.text = text
 ...
 export function parseGeneMsg(originMsg) {
   ...
   const { attachments, text } = originMsg
   const { payload = {}} = attachments[0]
   const { template_type = '', elements } = payload
   msgItem.text = text
   msgItem.type = template_type // 按钮模板和轮播图模板的 type 字段 由payload 中的template_type 决定
   msgItem.attachments = attachments
 ...
 export function parseError(originMsg) {
   // 错误信息默认当text文本处理
   const { error, msg } = originMsg
   const { type } = error
   const { pageId, senderId } = msg
 ...
 export function judgeType(parsedMsg) {
   ...
   switch (type) {
     ...
     case 'image': lastMsg = '发来了图片'; break
     case 'generic': lastMsg = '发来了轮播图'; break
     case 'button': lastMsg = '发来了按钮模板'; break
     default: return text
   }
   return lastMsg
 }
